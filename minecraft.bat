...
echo     d = {"nick": "PILOT_DASTR", "ram": "4"} >> %PY%
...
echo         print(f"   PILOT v4.0 | NICK: {c['nick']} | RAM: {c['ram']}G") >> %PY%
echo         print("==========================================") >> %PY%
...
echo                 print("\n[!] ГОТОВО! Нажми 3 чтобы играть!"); input() >> %PY%
echo             except Exception as e: print(e); input() >> %PY%
echo         elif m == "2": >> %PY%
echo             v = input("Версия ваниллы > "); start_game(v, GAME_DIR) >> %PY%
echo         elif m == "3": >> %PY%
...
echo             except: pass >> %PY%
echo         elif m == "5": c["nick"] = input("Ник: "); c["ram"] = input("ОЗУ: "); save_cfg(c) >> %PY%
